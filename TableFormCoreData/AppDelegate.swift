...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func verifyGender(){
        ...
        do{
            let entities = try context.fetch(request as! NSFetchRequest<NSFetchRequestResult>)
            if entities.count == 0 {
                let maledic:[String:AnyObject] = ["name":"male" as AnyObject]
                let femaledic:[String:AnyObject] = ["name":"female" as AnyObject]
                do{
                    _ = try Gender.findOrCreate(dic:maledic, in: context)
                    _ = try Gender.findOrCreate(dic:femaledic, in: context)
                    try context.save()
                } catch {
                    print(error)
                }
